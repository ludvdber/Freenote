...
/**
 * Admin authoring of guides. Sits under {@code /api/admin/**} → {@code hasRole("ADMIN")} (also
 * re-checked per request by {@code AdminRoleVerificationFilter}). The list/get here include drafts,
 * unlike the public {@link GuideController}.
 */
@RestController
...
public class AdminGuideController {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<GuideResponse> get(@PathVariable Long id) {
    ...
    public ResponseEntity<GuideResponse> create(Authentication authentication,
        ...
        return ResponseEntity.status(HttpStatus.CREATED).body(service.create(adminId, request));
    }

    @PutMapping("/{id}")
    public ResponseEntity<GuideResponse> update(@PathVariable Long id,
                                                @Valid @RequestBody CreateGuideRequest request) {
